...
module.exports = async (self, target, coins) => {
  if (isNaN(coins) || coins <= 0)
    return "Please enter a valid amount of coins to transfer - must be a number greater than 0";
  if (target.bot) return "You cannot transfer coins to bots!";
  if (target.id === self.id) return "You cannot transfer coins to self! - you can always withdraw them";

  const userDb = await getUser(self.id);

  if (userDb.coins < coins) {
    return `Insufficient coin balance! You only have ${userDb.coins}${ECONOMY.CURRENCY} - Run the \`${self.prefix}balance\` command to check your balance`;
  }
