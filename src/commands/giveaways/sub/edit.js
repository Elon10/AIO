...
module.exports = async (member, messageId, addDuration, newPrize, newWinnerCount) => {
  if (!messageId) return "You must provide a valid message id. - `!giveaway edit <message id>`";

  // Permissions
  if (!member.permissions.has("MANAGE_MESSAGES")) {
    return "You need to have the manage messages permissions to start giveaways. - `MANAGE_MESSAGES`";
  }
  ...
  if (!giveaway)
    return `Unable to find a giveaway for messageId: ${messageId} - Please ensure you are using the correct message id.`;
  ...
  try {
    ...
    return `An error occurred while updating the giveaway: ${error.message} - Please contact the bot owner.`;
  }
};
