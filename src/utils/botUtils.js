...
function validateConfig() {
  log(chalk.green(`Validating config.js and environment variables`));
  // Validate .env file
  if (!process.env.BOT_TOKEN) {
    error(chalk.red(`env: BOT_TOKEN cannot be empty`));
    process.exit();
  }
  if (!process.env.MONGO_CONNECTION) {
    error(chalk.red(`env: MONGO_CONNECTION cannot be empty`));
    process.exit();
  }
  if (config.DASHBOARD.enabled) {
    if (!process.env.BOT_SECRET) {
      error(chalk.red(`env: BOT_SECRET cannot be empty`));
      process.exit();
    }
    if (!process.env.SESSION_PASSWORD) {
      error(chalk.red(`env: SESSION_PASSWORD cannot be empty`));
      process.exit();
    }
  }
  if (!process.env.WEATHERSTACK_KEY) {
    warn(chalk.yellow(`env: WEATHERSTACK_KEY is missing. Weather command won't work`));
  }

  // Validate config.js file
  if (isNaN(config.CACHE_SIZE.GUILDS) || isNaN(config.CACHE_SIZE.USERS) || isNaN(config.CACHE_SIZE.MEMBERS)) {
    error(chalk.red(`config.js: CACHE_SIZE must be a positive integer`));
    process.exit();
  }
  if (!config.PREFIX) {
    error(chalk.red(`config.js: PREFIX cannot be empty`));
    process.exit();
  }
  if (config.DASHBOARD.enabled) {
    if (!config.DASHBOARD.baseURL || !config.DASHBOARD.failureURL || !config.DASHBOARD.port) {
      error(chalk.red(`config.js: DASHBOARD details cannot be empty`));
      process.exit();
    }
  }
  if (config.OWNER_IDS.length === 0) warn(chalk.yellow(`config.js: OWNER_IDS are empty`));
  if (!config.SUPPORT_SERVER) warn(chalk.yellow(`config.js: SUPPORT_SERVER is not provided`));
}
